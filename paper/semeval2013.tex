...
field") of L1 classifiers in an effort to find the best translation into all
five target languages jointly. This network has nodes that correspond to the
distributions produced by the \emph{L1} classifiers, given an input sentence,
...
or simple correlations, we expect to see some regularities in the translation
process. So by building a Markov network in which all of the classifiers can
communicate (see Figure \ref{fig:pentagram}), we want the nodes to be able to
influence the translation decisions of their neighbors, but only proportionally
to the correlation between the translations that we observe in the two
languages.

%% XXX: is there some measure of the correlation between discrete nominal
%% varibles better than joint probability?

We structure our joint inference as an optimization process, in which we want
to minimize two penalty functions. First, for each of the five \emph{L1}
classifiers, we get a unary potential function, which assigns a penalty to each
possible label for the given target language of that node.

(MATH GOES HERE)

phi = negative log of probability assigned to that label by the classifier,
given the features

(END MATH)

Secondly, for each pair of classifiers $(i,j)$ (\emph{i.e.}, each edge in the
graph), 

(MATH GOES HERE)

phi = weight times negative log of joint probability of any two labels for
languages i and j.
