...
% values.
% TODO: pick consistent terminology for the word that we're trying to translate
...
and Wu have shown how to use WSD techniques to improve even modern phrase-based
SMT systems \cite{carpuatpsd}, which can mitigate the problem of lexical
ambiguities through the use of a language model and phrase-tables containing
multi-word phrases. 
...
appropriate contexts. The five target languages of the shared task are Spanish,
...
the first answer is given more weight in the evaluation, and the scoring
...
WSD literature. The first system, L1 (``layer one"), uses maximum entropy
...
(XXX fix this up a bit) In the work of Levefer and Hoste, they used Google
Translate API to translate the source English sentences into four other
languages in $ S = \left\lbrace t \right\rbrace$, and extracted all
bag-of-words features. Since an online MT API is not easily available, we did
not rely on a complete MT system and only included the translations of head
word (XXX) as features.

%L2 translate source work into five target languages separately, while MRF translates jointly.
...
%We optimize assignments over the five output variables using loopy belief
%propagation 
...
%of the correlations between the target-langauge words.
...
%%CAN LIU @Alex:   PMI? (I think quite similar to joint prob, except the denomiator is devided by both freq)
%% Chi-squared ?
...
%minimize the sum of two kinds of penalty functions. 
...
$$
\phi_{ij}(l_i, l_j) = - log P(L_i = l_i, L_j = l_j)
$$
...
$$
\delta_{i \rightarrow j} = \min_{L_i}
$$
