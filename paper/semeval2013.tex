...
the \emph{best} evaluation,(XXX CAN) all outputs for an instance taken into account, weighted by the number of agreements among annotators, and penalized by the number of output. So that the \emph{best} evaluation encourages the system to output the ``best" answer. (XXX) The \emph{oof} evaluation is similar except that it is not penalized by the number of outputs.
Both evaluations also include a ``mode" variant, in which only the answer agreed most by annotators 
%popular of the gold-standard answers 
are considered valid and compared against.
...
the benifit of using multilingual information from the parallel corpura, even 
without translating the whole source sentence to use bag-of-words features.  (XXX CAN)
%that they are making use of the information from the multiple parallel corpora. 
