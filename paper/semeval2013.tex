...
came from either the L2 or the MRF classifier for both evaluations.
...
There remain a number of open questions about ways in which the MRF classifiers
could be improved. XXX(alexr): write more here.

- how to jointly disambiguate all the words in a sentence?

- what if we don't have totally parallel data for all of the languages? how are
  we going to estimate the parameters then?
...
Aside from improvements to the design of our CL-WSD system itself, we also want
to use it in a practical system for translating into under-resourced languages.
